...
`backend/consultant/` 是多 Agent 智能顾问后端，基于 OpenAI Agents SDK 构建三层 Agent 调度架构，对外（目前）尚无 FastAPI 入口（`api/` 目录只有 `__init__.py`）。
...
所有测试均为独立脚本（直接 `python` 运行，工作目录须为 `backend/consultant/`）：

```bash
# 测试 Agent 路由（含 consult_agent 和 navigation_agent 完整用例）
cd backend/consultant && python test/agent_router_test.py
